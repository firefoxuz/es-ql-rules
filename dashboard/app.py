...
RULES_BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'rules'))
...
def parse_esql_metadata(file_path):
    ...
    if mapping_match: metadata['mapping'] = mapping_match.group(1).strip()
    if id_match: metadata['rule_id'] = id_match.group(1).strip()
    if title_match: metadata['title'] = title_match.group(1).strip()
    if desc_match: metadata['description'] = desc_match.group(1).strip()
    if mitre_match: metadata['mitre'] = mitre_match.group(1).strip()
...
def test_rule():
    ...
    try:
        ...
        return jsonify({
            'status': 'success',
            'hits': hits_count,
            'details': res.get('values', [])[:10]  # Return first 10 for preview
        })
